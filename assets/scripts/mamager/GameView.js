...
(function () {
    cc.GameView = cc.Class({
        ...
        onLoad: function () {
            ...
            console.log("fix ios 14   1")
            if (cc.sys.os === cc.sys.OS_IOS && cc.sys.isBrowser) {
                console.log("fix ios 14   2")
            ...
            cc.game.on(cc.game.EVENT_SHOW, function () {
               cc.LobbyController.getInstance().getUserIP();
            ...
            if (response.M !== undefined && response.M.length > 0) {
                ...
                switch (m.M) {
                    ...
                    case cc.MethodHubOnName.OTHER_DEVICE:
                        //console.log('cc.MethodHubOnName.OTHER_DEVICE');
                        cc.PopupController.getInstance().showPopupMessage('Mất kết nối đến server! vui lòng đăng nhập lại');
                        cc.LobbyController.getInstance().callLogout();
                        if(cc.TaiXiuLive){
                            cc.TaiXiuLive.inGame.actBack();
                        }
                        break;
                    case cc.MethodHubOnName.LIVE_VIDEO:
                        console.log('cc.MethodHubOnName.LIVE_VIDEO');
                        if(cc.ChatController.getInstance()){
                            cc.ChatController.getInstance().setChatViewLiveGame();
                        }
                        break;                       
                    case cc.MethodHubOnName.BALANCE_UPDATE:
                        console.log('cc.MethodHubOnName.BALANCE_UPDATE');
                        let message = m.A[0];
                        cc.PopupController.getInstance().showPopupMessage(message);
                        cc.LobbyController.getInstance().refreshAccountInfo();
                        break;
                    case cc.MethodHubOnName.EFFECT_JACKPOT_ALL:
                        console.log('cc.MethodHubOnName.EFFECT_JACKPOT_ALL');
                        //console.log(JSON.stringify(m.A[0]));
