...
(function () {
    cc.XocDiaVipView = cc.Class({
        ...
        onLoad: function () {
            cc.XocDiaVipController.getInstance().setXocDiaVipView(this);
        ...
        onDestroy: function () {
            ...
            cc.XocDiaVipController.getInstance().setXocDiaVipView(null);

            if (cc.sys.isNative) {
                cc.assetManager.releaseAsset('xocdiavip/prefabs');
                cc.assetManager.releaseAsset('xocdiavip/images');
            }
        ...
        updateInfo: function (sessionInfo) {
            ...
            switch (sessionInfo.CurrentState) {
                case cc.XocDiaVipState.BETTING:

                    break;
                case cc.XocDiaVipState.OPEN_PLATE:

                    break;
                case cc.XocDiaVipState.SHOW_RESULT:
        ...
        connectHub: function () {
            ...
            negotiateCommand.execute(this, cc.SubdomainName.XOC_DIA_VIP);
        },

        reconnect: function () {
            this.lastTimeReconnect = (new Date()).getTime();
            this.xxHub.connect(this, cc.HubName.XocDiaVipHub, this.connectionToken, true);
        },
        ...
        onSlotsNegotiateResponse: function (response) {
            ...
            this.xxHub.connect(this, cc.HubName.XocDiaVipHub, response.ConnectionToken);
        },
                ...
                mArray.map(m => {
                    switch (m.M) {
                        ...
                        case cc.MethodHubOnName.SESSION_INFO:
                            //console.log(m.A[0]);
                            // cc.XocDiaVipController.getInstance().updateSessionId(m.A[0].SessionID);
                            cc.XocDiaVipController.getInstance().updateInfo(m.A[0], m.A[0].Phrase, null);
                            cc.XocDiaVipController.getInstance().updateInput(m.A[0].Phrase);
                            cc.XocDiaVipController.getInstance().updateResult("update_time", m.A[0]);
                            break;
                        //Lich su choi
                        case cc.MethodHubOnName.GAME_HISTORY:
                            cc.XocDiaVipController.getInstance().resetDraw();
                            //ve bang Soi Cau
                            cc.XocDiaVipController.getInstance().draw(m.A[0]);
                            break;
                        //thoi gian + trang thai phien
                        case cc.MethodHubOnName.START_ACTION_TIMER:
                            var data = m.A;
                            cc.XocDiaVipController.getInstance().updateInfo(data[0], data[2], data[1]);
                            // cc.XocDiaVipController.getInstance().updateResult(data[0].Players, data[0].GameLoop.Result, data[0].GameLoop.OriginResult, data[2]);
                            cc.XocDiaVipController.getInstance().updateResult(null, data[0].Result, data[0].Result.ChipsData, data[2]);
                            cc.XocDiaVipController.getInstance().updateInput(data[2]);
                            break;
                        //nguoi choi roi ban
                        case cc.MethodHubOnName.PLAYER_LEAVE:
                            //phuc vu cho bot Xoc Xoc -> can thoat nhieu ng choi
                            if (mArray.length === 0) {
                                cc.XocDiaVipController.getInstance().playerLeave(m.A);
                            }
                            break;

                        //cap nhat trang thai cua nguoi choi
                        case cc.MethodHubOnName.UPDATE_CONNECTION_STATUS:
                            cc.XocDiaVipController.getInstance().updateConnectionStatus(m.A);
                            break;

                        //cập nhật lại trạng thái player
                        case cc.MethodHubOnName.UPDATE_PLAYER_STATUS:
                            cc.XocDiaVipController.getInstance().updatePlayerStatus(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.JOIN_GAME:
                            var data = m.A[0];
                            var info = m.A[1];
                            cc.XocDiaVipController.getInstance().updateInfoCurrPlayer(data.Account);
                            cc.PopupController.getInstance().hideBusy();
                            break;
                        //Tai hien cac cua da dat
                        case cc.MethodHubOnName.BET_SESSION:
                            cc.XocDiaVipController.getInstance().showLastInput(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.PLAYER_BET:
                            var data = m.A;
                            cc.XocDiaVipController.getInstance().playerBet(data);
                            break;
                        ...
                        case cc.MethodHubOnName.WIN_RESULT_VIP:
                            if (m.A.length > 0) {
                                try {
                                    setTimeout(function () {
                                        cc.XocDiaVipController.getInstance().winResultVip(m.A[0]);
                                    }, 2500);
                                } catch (e) {
                        ...
                        case cc.MethodHubOnName.WIN_RESULT:
                            if (m.A.length > 0) {
                                try {
                                    setTimeout(function () {
                                        cc.XocDiaVipController.getInstance().winResult(m.A[0]);
                                    }, 2500);
                                } catch (e) {
                        ...
                        case cc.MethodHubOnName.TOTAL_WIN_MONEY:
                            if(m.A[0] > 0) {
                                setTimeout(function () {
                                    cc.XocDiaVipController.getInstance().totalUserWin(m.A[0]);
                                }, 2500);
                            }
                            break;
                        ...
                        case cc.MethodHubOnName.OPEN_PLATE_NOW:
                            cc.XocDiaVipController.getInstance().updateResult(null, m.A[0], m.A[1], cc.XocDiaVipState.OPEN_PLATE, true);
                            cc.XocDiaVipController.getInstance().updateInput(cc.XocDiaVipState.OPEN_PLATE);
                            break;

                        //nhan message chat
                        case cc.MethodHubOnName.RECEIVE_MESSAGE:
                            cc.ChatRoomController.getInstance().addChatContent(m.A);
                            cc.XocDiaVipController.getInstance().playerShowBubbleChat(m.A);
                            break;
                        case cc.MethodHubOnName.SUMMARY_PLAYER:
                            cc.XocDiaVipController.getInstance().summaryPlayer(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.VIP_PLAYERS:
                            ...
                            if (dataPlayers.length > 0) {
                                cc.XocDiaVipController.getInstance().vipPlayer(dataPlayers);
                            }
                            break;
                        //FIX BUG
                        case 'recieveMessage':
                        ...
                        case cc.MethodHubOnName.UPDATE_ROOM_TIME:
                            cc.XocDiaVipController.getInstance().updateTimer(m.A[0]);
        ...
        helpClicked: function () {
            cc.XocDiaVipPopupController.getInstance().createHelpView();
        },

        //lich su dat cuoc
        historyClicked: function () {
            cc.XocDiaVipPopupController.getInstance().createHistoryView();
        },

        //bang xep hang dat cuoc
        topClicked: function () {
            cc.XocDiaVipPopupController.getInstance().createTopView();
        },

        //bieu do chi tiet cac phien
        graphClicked: function () {
            cc.XocDiaVipPopupController.getInstance().createGraphView();
        },
