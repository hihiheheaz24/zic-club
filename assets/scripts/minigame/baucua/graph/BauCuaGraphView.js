        ...
        onResponse: function (list) {
            if (list) {
                ...
                for (i = list.length - 1; i >= 0; i--) {
                    this.calcSumDice(list[i].FirstDice);
                    this.calcSumDice(list[i].SecondDice);
                    this.calcSumDice(list[i].ThirdDice);
                    //map item

                    let itemDiceUI = this.poolItemDice.get();
                    if (i == 0) {
                        itemDiceUI.opacity = 255;
                    } else {
                        itemDiceUI.opacity = 155;
                    }
                    itemDiceUI.getComponent('BauCuaGraphItem').setResultDice(list[i], i);
