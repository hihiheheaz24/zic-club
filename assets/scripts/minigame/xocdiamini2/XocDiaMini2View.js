...
(function () {
    cc.XocDiaMini2View = cc.Class({
        ...
        onLoad: function () {
            cc.XocDiaMini2Controller.getInstance().setXocDiaMini2View(this);
        ...
        onDestroy: function () {
            ...
            cc.XocDiaMini2Controller.getInstance().setXocDiaMini2View(null);

            if (cc.sys.isNative) {
                cc.assetManager.releaseAsset('xocdiamini2/prefabs');
                cc.assetManager.releaseAsset('xocdiamini2/images');
            }
        ...
        updateInfo: function (sessionInfo) {
            ...
            switch (sessionInfo.CurrentState) {
                case cc.XocDiaMini2State.BETTING:

                    break;
                case cc.XocDiaMini2State.OPEN_PLATE:

                    break;
                case cc.XocDiaMini2State.SHOW_RESULT:

                    break;
                case cc.XocDiaMini2State.WAITING:

                    break;
                case cc.XocDiaMini2State.SHAKING:
        ...
        connectHub: function () {
            ...
            negotiateCommand.execute(this, cc.SubdomainName.XOC_DIA_MINI2);
        },

        reconnect: function () {
            this.lastTimeReconnect = (new Date()).getTime();
            this.xxHub.connect(this, cc.HubName.XocDiaMini2Hub, this.connectionToken, true);
        },
        ...
        onSlotsNegotiateResponse: function (response) {
            ...
            this.xxHub.connect(this, cc.HubName.XocDiaMini2Hub, response.ConnectionToken);
        },
                ...
                mArray.map(m => {
                    switch (m.M) {
                        ...
                        case cc.MethodHubOnName.SESSION_INFO:
                            // cc.XocDiaMini2Controller.getInstance().updateSessionId(m.A[0].SessionID);
                            cc.XocDiaMini2Controller.getInstance().updateInfo(m.A[0], m.A[0].Phrase, null);
                            cc.XocDiaMini2Controller.getInstance().updateInput(m.A[0].Phrase);
                            cc.XocDiaMini2Controller.getInstance().updateResult("update_time", m.A[0]);
                            break;
                        //Lich su choi
                        case cc.MethodHubOnName.GAME_HISTORY:
                            cc.XocDiaMini2Controller.getInstance().resetDraw();
                            //ve bang Soi Cau
                            cc.XocDiaMini2Controller.getInstance().draw(m.A[0]);
                            break;
                        //thoi gian + trang thai phien
                        case cc.MethodHubOnName.START_ACTION_TIMER:
                            var data = m.A;
                            cc.XocDiaMini2Controller.getInstance().updateInfo(data[0], data[2], data[1]);
                            // cc.XocDiaMini2Controller.getInstance().updateResult(data[0].Players, data[0].GameLoop.Result, data[0].GameLoop.OriginResult, data[2]);
                            cc.XocDiaMini2Controller.getInstance().updateResult(null, data[0].Result, data[0].Result.ChipsData, data[2]);
                            cc.XocDiaMini2Controller.getInstance().updateInput(data[2]);
                            break;
                        //nguoi choi roi ban
                        case cc.MethodHubOnName.PLAYER_LEAVE:
                            //phuc vu cho bot Xoc Xoc -> can thoat nhieu ng choi
                            if (mArray.length === 0) {
                                cc.XocDiaMini2Controller.getInstance().playerLeave(m.A);
                            }
                            break;

                        //cap nhat trang thai cua nguoi choi
                        case cc.MethodHubOnName.UPDATE_CONNECTION_STATUS:
                            cc.XocDiaMini2Controller.getInstance().updateConnectionStatus(m.A);
                            break;

                        //cập nhật lại trạng thái player
                        case cc.MethodHubOnName.UPDATE_PLAYER_STATUS:
                            cc.XocDiaMini2Controller.getInstance().updatePlayerStatus(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.JOIN_GAME:
                            var data = m.A[0];
                            var info = m.A[1];
                            cc.XocDiaMini2Controller.getInstance().updateInfoCurrPlayer(data.Account);
                            cc.PopupController.getInstance().hideBusy();
                            break;
                        //Tai hien cac cua da dat
                        case cc.MethodHubOnName.BET_SESSION:
                            cc.XocDiaMini2Controller.getInstance().showLastInput(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.PLAYER_BET:
                            var data = m.A;
                            cc.XocDiaMini2Controller.getInstance().playerBet(data);
                            break;
                        ...
                        case cc.MethodHubOnName.WIN_RESULT_VIP:
                            if (m.A.length > 0) {
                                try {
                                    setTimeout(function () {
                                        cc.XocDiaMini2Controller.getInstance().winResultVip(m.A[0]);
                                    }, 2500);
                                } catch (e) {
                        ...
                        case cc.MethodHubOnName.WIN_RESULT:
                            if (m.A.length > 0) {
                                try {
                                    setTimeout(function () {
                                        cc.XocDiaMini2Controller.getInstance().winResult(m.A[0]);
                                    }, 2500);
                                } catch (e) {
                        ...
                        case cc.MethodHubOnName.TOTAL_WIN_MONEY:
                            if(m.A[0] > 0) {
                                setTimeout(function () {
                                    cc.XocDiaMini2Controller.getInstance().totalUserWin(m.A[0]);
                                }, 2500);
                            }
                            break;
                        ...
                        case cc.MethodHubOnName.OPEN_PLATE_NOW:
                            cc.XocDiaMini2Controller.getInstance().updateResult(null, m.A[0], m.A[1], cc.XocDiaMini2State.OPEN_PLATE, true);
                            cc.XocDiaMini2Controller.getInstance().updateInput(cc.XocDiaMini2State.OPEN_PLATE);
                            break;

                        //nhan message chat
                        case cc.MethodHubOnName.RECEIVE_MESSAGE:
                            cc.ChatRoomController.getInstance().addChatContent(m.A);
                            cc.XocDiaMini2Controller.getInstance().playerShowBubbleChat(m.A);
                            break;
                        case cc.MethodHubOnName.SUMMARY_PLAYER:
                            cc.XocDiaMini2Controller.getInstance().summaryPlayer(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.VIP_PLAYERS:
                            ...
                            if (dataPlayers.length > 0) {
                                cc.XocDiaMini2Controller.getInstance().vipPlayer(dataPlayers);
                            }
                            break;
                        //FIX BUG
                        case 'recieveMessage':
                        ...
                        case cc.MethodHubOnName.UPDATE_ROOM_TIME:
                            cc.XocDiaMini2Controller.getInstance().updateTimer(m.A[0]);
        ...
        helpClicked: function () {
            cc.XocDiaMini2PopupController.getInstance().createHelpView();
        },

        //lich su dat cuoc
        historyClicked: function () {
            cc.XocDiaMini2PopupController.getInstance().createHistoryView();
        },

        //bang xep hang dat cuoc
        topClicked: function () {
            cc.XocDiaMini2PopupController.getInstance().createTopView();
        },

        //bieu do chi tiet cac phien
        graphClicked: function () {
            cc.XocDiaMini2PopupController.getInstance().createGraphView();
        },
