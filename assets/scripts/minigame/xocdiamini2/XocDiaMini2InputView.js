...
const players = require('XocDiaMini2PlayerData').players;

(function () {
    cc.XocDiaMini2InputView = cc.Class({
        ...
        onLoad: function () {
            cc.XocDiaMini2Controller.getInstance().setXocDiaMini2InputView(this);

            //danh dau che do Nan
            this.isNan = false;
            cc.XocDiaMini2Controller.getInstance().setIsNan(this.isNan);
            ...
            //reset lastBetData
            cc.XocDiaMini2Controller.getInstance().setLastBetData(null);
            ...
            this.initGateChip();
            cc.XocDiaMini2Controller.getInstance().initLogBet();
        },
		update: function () {
        ...
        playerBet: function (info) {
            ...
            if (cc.XocDiaMini2Controller.getInstance().getPositions()) {
                var accID = info[0];
                var amount = info[1];
                var gate = info[2];
                var chip = info[3];

                cc.XocDiaMini2Controller.getInstance().updateChip(accID, chip);

                /* this.playFxUserBet(
                    cc.XocDiaMini2Controller.getInstance().getIndexUIBetByAccID(accID),
                    gate,
                ...
                if (accID === cc.LoginController.getInstance().getUserId()) {
                    cc.XocDiaMini2Controller.getInstance().setLogBet({
                        'AccountID': accID,
                        'Amount': amount,
                        'Gate': gate
                    });
                    ...
                    cc.DDNA.getInstance().betSummary(cc.DDNAGame.XOC_DIA_MINI, amount, cc.XocDiaMini2Controller.getInstance().getSID());
                }
            }
        },
        ...
        reBet: function (betLog, isX2) {
            ...
            betLog.forEach(function (bet) {
                switch (bet.Gate) {
                    case cc.XocDiaMini2Gate.ODD:
                        ODD += bet.Amount;
                        break;
                    case cc.XocDiaMini2Gate.THREE_UP:
                        THREE_UP += bet.Amount;
                        break;
                    case cc.XocDiaMini2Gate.THREE_DOWN:
                        THREE_DOWN += bet.Amount;
                        break;
                    case cc.XocDiaMini2Gate.EVEN:
                        EVEN += bet.Amount;
                        break;
                    case cc.XocDiaMini2Gate.FOUR_UP:
                        FOUR_UP += bet.Amount;
                        break;
                    case cc.XocDiaMini2Gate.FOUR_DOWN:
                        FOUR_DOWN += bet.Amount;
                        break;
                    case cc.XocDiaMini2Gate.TWO:
                        TWO += bet.Amount;
                        break;
                }
            });
            ...
            bets.forEach(function (bet) {
                self.timeouts.push(
                    setTimeout(function () {
                        if (self.currentState === cc.XocDiaMini2State.BETTING) {
        ...
        sendRequestReBet: function (bet) {
            ...
            if (cc.BalanceController.getInstance().getBalance() < bet.Amount) {
                ...
                //send request
                cc.XocDiaMini2Controller.getInstance().sendRequestOnHub(cc.MethodHubName.BET, bet.Amount, bet.Gate);
            }
        },

        showLastInput: function (info) {
            // console.log('XocDiaMini2Input showLastInput');
            var self = this;
            var betLogs = info;
            ...
            betLogs.forEach(function (betLog) {
                ...
                betLog.forEach(function (bet) {
                    self.playFxUserBet(
                        cc.XocDiaMini2Controller.getInstance().getIndexUIBetByAccID(bet.AccountID),
                        bet.BetSide,
        ...
        resetInput: function () {
            // console.log('XocDiaMini2Input resetInput');
        ...
        saveLastBetData: function () {
            ...
            let logBet = [...cc.XocDiaMini2Controller.getInstance().getLogBet()];

            cc.XocDiaMini2Controller.getInstance().setLastBetData(logBet);
        },
        ...
        updateInput: function (state) {
            ...
            switch (state) {
                //giai doan dat cuoc
                case cc.XocDiaMini2State.BETTING: //54
                ...
                //giai doan ket qua
                case cc.XocDiaMini2State.SHOW_RESULT: //15
                    if (this.currentState !== state) {
                        this.activeAllButtonBet(false);
                    }
                    break;
        ...
        playFxUserBet: function (playerId, gate, chipIndex, isMove) {
            ...
            var nodeChip = cc.XocDiaMini2Controller.getInstance().createChip();
            ...
            var chipItem = nodeChip.getComponent(cc.XocDiaMini2ChipItem);
            //set vi tri bet
            chipItem.betIndex = betIndex;
            chipItem.gate = gate;
            chipItem.playerId = playerId;
        ...
        betClicked: function (event, data) {
            if (cc.XocDiaMini2Controller.getInstance().getTime() <= 3) {
                cc.PopupController.getInstance().showMessage('Đã hết thời gian đặt cửa.');
                cc.XocDiaMini2Controller.getInstance().activeAllButtonBet(false);
                return;
            }
 ...
 */
            ...
            if (cc.BalanceController.getInstance().getBalance() < betVal) {
                ...
                cc.XocDiaMini2Controller.getInstance().sendRequestOnHub(cc.MethodHubName.BET, betVal, this.indexBet + 1);
        ...
        nanClicked: function () {
            ...
            if (this.isNan) {
                this.spriteNan.spriteFrame = cc.XocDiaMini2Controller.getInstance().getNans()[0];
            } else {
                this.spriteNan.spriteFrame = cc.XocDiaMini2Controller.getInstance().getNans()[1];
            }
        ...
        x2Clicked: function () {
            ...
            var lastBetData = cc.XocDiaMini2Controller.getInstance().getLastBetData();
