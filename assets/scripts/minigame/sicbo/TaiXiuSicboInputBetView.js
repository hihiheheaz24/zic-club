...
(function () {
  cc.TaiXiuSicboInputBetView = cc.Class({
    ...
    //         // if(!bot.getComponentInChildren(cc.Label)){
    //         //   console.log("rand2"+rand2);
    //         //   return;
    //         // };
    //         let botBet = Number(
    //           this.getBotMoney(bot)
    //         );
    //         if(!this.chip2s[rand3]) return;
    ...
    //           this.id2 = setTimeout(
    //             function () {
    //               if(!this.sprBotAvatars) return;
    //               var rndSpr = Math.floor(
    //                 Math.random() * (this.sprBotAvatars.length - 0 + 1) + 0
    //               );
    //               if(this.ignoreBot > 5 || this.ignoreBot < 0) return;
