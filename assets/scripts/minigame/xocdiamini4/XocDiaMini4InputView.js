...
const players = require('XocDiaMini4PlayerData').players;

(function () {
    cc.XocDiaMini4InputView = cc.Class({
        ...
        onLoad: function () {
            cc.XocDiaMini4Controller.getInstance().setXocDiaMini4InputView(this);

            //danh dau che do Nan
            this.isNan = false;
            cc.XocDiaMini4Controller.getInstance().setIsNan(this.isNan);
            ...
            //reset lastBetData
            cc.XocDiaMini4Controller.getInstance().setLastBetData(null);
            ...
            this.initGateChip();
            cc.XocDiaMini4Controller.getInstance().initLogBet();
        },
		update: function () {
        ...
        playerBet: function (info) {
            ...
            if (cc.XocDiaMini4Controller.getInstance().getPositions()) {
                var accID = info[0];
                var amount = info[1];
                var gate = info[2];
                var chip = info[3];

                cc.XocDiaMini4Controller.getInstance().updateChip(accID, chip);

                /* this.playFxUserBet(
                    cc.XocDiaMini4Controller.getInstance().getIndexUIBetByAccID(accID),
                    gate,
                ...
                if (accID === cc.LoginController.getInstance().getUserId()) {
                    cc.XocDiaMini4Controller.getInstance().setLogBet({
                        'AccountID': accID,
                        'Amount': amount,
                        'Gate': gate
                    });
                    ...
                    cc.DDNA.getInstance().betSummary(cc.DDNAGame.XOC_DIA_MINI, amount, cc.XocDiaMini4Controller.getInstance().getSID());
                }
            }
        },
        ...
        reBet: function (betLog, isX2) {
            ...
            bets.forEach(function (bet) {
                self.timeouts.push(
                    setTimeout(function () {
                        if (self.currentState === cc.XocDiaMini4State.BETTING) {
        ...
        sendRequestReBet: function (bet) {
            ...
            if (cc.BalanceController.getInstance().getBalance() < bet.Amount) {
                ...
                //send request
                cc.XocDiaMini4Controller.getInstance().sendRequestOnHub(cc.MethodHubName.BET, bet.Amount, bet.Gate);
            }
        },

        showLastInput: function (info) {
            // console.log('XocDiaMini4Input showLastInput');
            var self = this;
            var betLogs = info;
            ...
            betLogs.forEach(function (betLog) {
                ...
                betLog.forEach(function (bet) {
                    self.playFxUserBet(
                        cc.XocDiaMini4Controller.getInstance().getIndexUIBetByAccID(bet.AccountID),
                        bet.BetSide,
        ...
        resetInput: function () {
            // console.log('XocDiaMini4Input resetInput');
        ...
        saveLastBetData: function () {
            ...
            let logBet = [...cc.XocDiaMini4Controller.getInstance().getLogBet()];

            cc.XocDiaMini4Controller.getInstance().setLastBetData(logBet);
        },
        ...
        playFxUserBet: function (playerId, gate, chipIndex, isMove) {
            ...
            var nodeChip = cc.XocDiaMini4Controller.getInstance().createChip();
            ...
            var chipItem = nodeChip.getComponent(cc.XocDiaMini4ChipItem);
            //set vi tri bet
            chipItem.betIndex = betIndex;
            chipItem.gate = gate;
            chipItem.playerId = playerId;
        ...
        betClicked: function (event, data) {
            if (cc.XocDiaMini4Controller.getInstance().getTime() <= 3) {
            ...
            if (cc.BalanceController.getInstance().getBalance() < betVal) {
                ...
                cc.XocDiaMini4Controller.getInstance().sendRequestOnHub(cc.MethodHubName.BET, betVal, this.indexBet + 1);
        ...
        nanClicked: function () {
            ...
            if (this.isNan) {
                this.spriteNan.spriteFrame = cc.XocDiaMini4Controller.getInstance().getNans()[0];
            } else {
                this.spriteNan.spriteFrame = cc.XocDiaMini4Controller.getInstance().getNans()[1];
            }
        ...
        x2Clicked: function () {
            ...
            var lastBetData = cc.XocDiaMini4Controller.getInstance().getLastBetData();
