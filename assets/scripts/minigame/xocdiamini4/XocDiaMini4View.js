...
(function () {
    cc.XocDiaMini4View = cc.Class({
        ...
        onLoad: function () {
            cc.XocDiaMini4Controller.getInstance().setXocDiaMini4View(this);
        ...
        onDestroy: function () {
            ...
            cc.XocDiaMini4Controller.getInstance().setXocDiaMini4View(null);

            if (cc.sys.isNative) {
                cc.assetManager.releaseAsset('xocdiamini4/prefabs');
                cc.assetManager.releaseAsset('xocdiamini4/images');
            }
        ...
        connectHub: function () {
            ...
            negotiateCommand.execute(this, cc.SubdomainName.XOC_DIA_MINI4);
        },

        reconnect: function () {
            this.lastTimeReconnect = (new Date()).getTime();
            this.xxHub.connect(this, cc.HubName.XocDiaMini4Hub, this.connectionToken, true);
        },
        ...
        onSlotsNegotiateResponse: function (response) {
            ...
            this.xxHub.connect(this, cc.HubName.XocDiaMini4Hub, response.ConnectionToken);
        },
                ...
                mArray.map(m => {
                    switch (m.M) {
                        ...
                        case cc.MethodHubOnName.SESSION_INFO:
                            // cc.XocDiaMini4Controller.getInstance().updateSessionId(m.A[0].SessionID);
                            cc.XocDiaMini4Controller.getInstance().updateInfo(m.A[0], m.A[0].Phrase, null);
                            cc.XocDiaMini4Controller.getInstance().updateInput(m.A[0].Phrase);
                            cc.XocDiaMini4Controller.getInstance().updateResult("update_time", m.A[0]);
                            break;
                        //Lich su choi
                        case cc.MethodHubOnName.GAME_HISTORY:
                            cc.XocDiaMini4Controller.getInstance().resetDraw();
                            //ve bang Soi Cau
                            cc.XocDiaMini4Controller.getInstance().draw(m.A[0]);
                            break;
                        //thoi gian + trang thai phien
                        case cc.MethodHubOnName.START_ACTION_TIMER:
                            var data = m.A;
                            cc.XocDiaMini4Controller.getInstance().updateInfo(data[0], data[2], data[1]);
                            // cc.XocDiaMini4Controller.getInstance().updateResult(data[0].Players, data[0].GameLoop.Result, data[0].GameLoop.OriginResult, data[2]);
                            cc.XocDiaMini4Controller.getInstance().updateResult(null, data[0].Result, data[0].Result.ChipsData, data[2]);
                            cc.XocDiaMini4Controller.getInstance().updateInput(data[2]);
                            break;
                        //nguoi choi roi ban
                        case cc.MethodHubOnName.PLAYER_LEAVE:
                            //phuc vu cho bot Xoc Xoc -> can thoat nhieu ng choi
                            if (mArray.length === 0) {
                                cc.XocDiaMini4Controller.getInstance().playerLeave(m.A);
                            }
                            break;

                        //cap nhat trang thai cua nguoi choi
                        case cc.MethodHubOnName.UPDATE_CONNECTION_STATUS:
                            cc.XocDiaMini4Controller.getInstance().updateConnectionStatus(m.A);
                            break;

                        //cập nhật lại trạng thái player
                        case cc.MethodHubOnName.UPDATE_PLAYER_STATUS:
                            cc.XocDiaMini4Controller.getInstance().updatePlayerStatus(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.JOIN_GAME:
                            var data = m.A[0];
                            var info = m.A[1];
                            cc.XocDiaMini4Controller.getInstance().updateInfoCurrPlayer(data.Account);
                            cc.PopupController.getInstance().hideBusy();
                            break;
                        //Tai hien cac cua da dat
                        case cc.MethodHubOnName.BET_SESSION:
                            cc.XocDiaMini4Controller.getInstance().showLastInput(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.PLAYER_BET:
                            var data = m.A;
                            cc.XocDiaMini4Controller.getInstance().playerBet(data);
                            break;
                        ...
                        case cc.MethodHubOnName.WIN_RESULT_VIP:
                            if (m.A.length > 0) {
                                try {
                                    setTimeout(function () {
                                        cc.XocDiaMini4Controller.getInstance().winResultVip(m.A[0]);
                                    }, 2500);
                                } catch (e) {
                        ...
                        case cc.MethodHubOnName.WIN_RESULT:
                            if (m.A.length > 0) {
                                try {
                                    setTimeout(function () {
                                        cc.XocDiaMini4Controller.getInstance().winResult(m.A[0]);
                                    }, 2500);
                                } catch (e) {
                        ...
                        case cc.MethodHubOnName.TOTAL_WIN_MONEY:
                            if(m.A[0] > 0) {
                                setTimeout(function () {
                                    cc.XocDiaMini4Controller.getInstance().totalUserWin(m.A[0]);
                                }, 2500);
                            }
                            break;
                        ...
                        case cc.MethodHubOnName.OPEN_PLATE_NOW:
                            cc.XocDiaMini4Controller.getInstance().updateResult(null, m.A[0], m.A[1], cc.XocDiaMini4State.OPEN_PLATE, true);
                            cc.XocDiaMini4Controller.getInstance().updateInput(cc.XocDiaMini4State.OPEN_PLATE);
                            break;

                        //nhan message chat
                        case cc.MethodHubOnName.RECEIVE_MESSAGE:
                            cc.ChatRoomController.getInstance().addChatContent(m.A);
                            cc.XocDiaMini4Controller.getInstance().playerShowBubbleChat(m.A);
                            break;
                        case cc.MethodHubOnName.SUMMARY_PLAYER:
                            cc.XocDiaMini4Controller.getInstance().summaryPlayer(m.A[0]);
                            break;
                        ...
                        case cc.MethodHubOnName.VIP_PLAYERS:
                            ...
                            if (dataPlayers.length > 0) {
                                cc.XocDiaMini4Controller.getInstance().vipPlayer(dataPlayers);
                            }
                            break;
                        //FIX BUG
                        case 'recieveMessage':
                        ...
                        case cc.MethodHubOnName.UPDATE_ROOM_TIME:
                            cc.XocDiaMini4Controller.getInstance().updateTimer(m.A[0]);
        ...
        helpClicked: function () {
            cc.XocDiaMini4PopupController.getInstance().createHelpView();
        },

        //lich su dat cuoc
        historyClicked: function () {
            cc.XocDiaMini4PopupController.getInstance().createHistoryView();
        },

        //bang xep hang dat cuoc
        topClicked: function () {
            cc.XocDiaMini4PopupController.getInstance().createTopView();
        },

        //bieu do chi tiet cac phien
        graphClicked: function () {
            cc.XocDiaMini4PopupController.getInstance().createGraphView();
        },
		...
		showNodeLon: function () {
			...
			cc.XocDiaMiniController.getInstance().getSetPositionNodeNhoTop1();
