...
const players = require('XocDiaMiniPlayerData').players;

(function () {
    cc.XocDiaMiniInputView = cc.Class({
        ...
        onLoad: function () {
            cc.XocDiaMiniController.getInstance().setXocDiaMiniInputView(this);

            //danh dau che do Nan
            this.isNan = false;
            cc.XocDiaMiniController.getInstance().setIsNan(this.isNan);
            ...
            this.betVals = [10000, 50000, 100000, 200000, 500000,1000000,2000000];
            this.processBetValUI();

            //reset lastBetData
            cc.XocDiaMiniController.getInstance().setLastBetData(null);
            ...
            this.initGateChip();
            cc.XocDiaMiniController.getInstance().initLogBet();
        },
		update: function () {
        ...
        playerBet: function (info) {
            ...
            if (cc.XocDiaMiniController.getInstance().getPositions()) {
                var accID = info[0];
                var amount = info[1];
                var gate = info[2];
                var chip = info[3];

                cc.XocDiaMiniController.getInstance().updateChip(accID, chip);

                /* this.playFxUserBet(
                    cc.XocDiaMiniController.getInstance().getIndexUIBetByAccID(accID),
                    gate,
                ...
                if (accID === cc.LoginController.getInstance().getUserId()) {
                    cc.XocDiaMiniController.getInstance().setLogBet({
                        'AccountID': accID,
                        'Amount': amount,
                        'Gate': gate
                    });
                    ...
                    cc.DDNA.getInstance().betSummary(cc.DDNAGame.XOC_DIA_MINI, amount, cc.XocDiaMiniController.getInstance().getSID());
                }
            }
        },
        ...
        reBet: function (betLog, isX2) {
            ...
            betLog.forEach(function (bet) {
                switch (bet.Gate) {
                    case cc.XocDiaMiniGate.ODD:
                        ODD += bet.Amount;
                        break;
                    case cc.XocDiaMiniGate.THREE_UP:
                        THREE_UP += bet.Amount;
                        break;
                    case cc.XocDiaMiniGate.THREE_DOWN:
                        THREE_DOWN += bet.Amount;
                        break;
                    case cc.XocDiaMiniGate.EVEN:
                        EVEN += bet.Amount;
                        break;
                    case cc.XocDiaMiniGate.FOUR_UP:
                        FOUR_UP += bet.Amount;
                        break;
                    case cc.XocDiaMiniGate.FOUR_DOWN:
                        FOUR_DOWN += bet.Amount;
                        break;
                    case cc.XocDiaMiniGate.TWO:
                        TWO += bet.Amount;
                        break;
                }
            });
            ...
            bets.forEach(function (bet) {
                self.timeouts.push(
                    setTimeout(function () {
                        if (self.currentState === cc.XocDiaMiniState.BETTING) {
        ...
        sendRequestReBet: function (bet) {
            ...
            if (cc.BalanceController.getInstance().getBalance() < bet.Amount) {
                ...
                //send request
                cc.XocDiaMiniController.getInstance().sendRequestOnHub(cc.MethodHubName.BET, bet.Amount, bet.Gate);
            }
        },

        showLastInput: function (info) {
            // console.log('XocDiaMiniInput showLastInput');
            var self = this;
            var betLogs = info;
            ...
            betLogs.forEach(function (betLog) {
                ...
                betLog.forEach(function (bet) {
                    self.playFxUserBet(
                        cc.XocDiaMiniController.getInstance().getIndexUIBetByAccID(bet.AccountID),
                        bet.BetSide,
        ...
        resetInput: function () {
            // console.log('XocDiaMiniInput resetInput');
        ...
        saveLastBetData: function () {
            ...
            let logBet = [...cc.XocDiaMiniController.getInstance().getLogBet()];

            cc.XocDiaMiniController.getInstance().setLastBetData(logBet);
        },
        ...
        updateInput: function (state) {
            ...
            switch (state) {
                //giai doan dat cuoc
                case cc.XocDiaMiniState.BETTING: //54
                ...
                //giai doan ket qua
                case cc.XocDiaMiniState.SHOW_RESULT: //15
                    if (this.currentState !== state) {
                        this.activeAllButtonBet(false);
                    }
                    break;
        ...
        playFxUserBet: function (playerId, gate, chipIndex, isMove) {
            ...
            var nodeChip = cc.XocDiaMiniController.getInstance().createChip();
            ...
            var chipItem = nodeChip.getComponent(cc.XocDiaMiniChipItem);
            //set vi tri bet
            chipItem.betIndex = betIndex;
            chipItem.gate = gate;
            chipItem.playerId = playerId;
        ...
        betClicked: function (event, data) {
            if (cc.XocDiaMiniController.getInstance().getTime() <= 3) {
                cc.PopupController.getInstance().showMessage('Đã hết thời gian đặt cửa.');
                cc.XocDiaMiniController.getInstance().activeAllButtonBet(false);
                return;
            }
            ...
            if (cc.BalanceController.getInstance().getBalance() < betVal) {
                ...
                cc.XocDiaMiniController.getInstance().sendRequestOnHub(cc.MethodHubName.BET, betVal, this.indexBet + 1);
        ...
        nanClicked: function () {
            ...
            if (this.isNan) {
                this.spriteNan.spriteFrame = cc.XocDiaMiniController.getInstance().getNans()[0];
            } else {
                this.spriteNan.spriteFrame = cc.XocDiaMiniController.getInstance().getNans()[1];
            }
        ...
        x2Clicked: function () {
            ...
            var lastBetData = cc.XocDiaMiniController.getInstance().getLastBetData();
