...
(function () {
    cc.XocDiaMiniResultView = cc.Class({
        ...
        onLoad: function () {
            cc.XocDiaMiniController.getInstance().setXocDiaMiniResultView(this);
        ...
        updateResult: function (players, result, originResult, state, openNow) {
            if (players == "update_time" && result["Phrase"] == cc.XocDiaMiniState.BETTING) {
                if (result["Elapsed"] < 5) {
                    [
                        this.nodeChan1,
                        this.nodeChan2,
            ...
            switch (state) {
                ...
                case cc.XocDiaMiniState.WAITING:
                    if (this.currentState !== state) {
                        cc.XocDiaMiniController.getInstance().initGateChip();
        ...
        playFxResult: function (result, originResult, openNow) {
            ...
            //dang o che do Nan
            if (cc.XocDiaMiniController.getInstance().getIsNan() && !openNow) {
                //bat bat Nan
                this.nodeDia.active = true;
        ...
        playPayFx: function (players, result) {
            ...
            let gateChips = cc.XocDiaMiniController.getInstance().getGateChips();
            var self = this;
            ...
            this.fxMoveChip(arrGateLose, cc.XocDiaMini_FX.LOSE);
            ...
            setTimeout(function () {
                this.fxMoveChip(arrGateWin, cc.XocDiaMini_FX.DEALER_PAY);
            }.bind(this), 1000);
            ...
            setTimeout(function () {
                this.fxMoveChip(arrGateWin, cc.XocDiaMini_FX.PAY);
            }.bind(this), 2000); */
                ...
                arrGate.map(gateIndex => {
                    if (gateChips[gateIndex] && gateChips[gateIndex].length) {
                        gateChips[gateIndex].forEach(function (chip) {
                            switch (typeFx) {
                                case cc.XocDiaMini_FX.LOSE:
                                    cc.XocDiaMiniController.getInstance().playFxLost(chip);
                                    break;
                                case cc.XocDiaMini_FX.DEALER_PAY:
                                    cc.XocDiaMiniController.getInstance().playFxDealerPay(chip);
                                    break;
                                case cc.XocDiaMini_FX.PAY:
                                    cc.XocDiaMiniController.getInstance().playFxPay(chip);
                                    break;
                            }
                        })
                    }
