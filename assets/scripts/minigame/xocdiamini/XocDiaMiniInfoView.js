...
(function () {
    cc.XocDiaMiniInfoView = cc.Class({
        ...
        onLoad: function () {
            ...
            this.timeBet = 54;
            this.reset();
            cc.XocDiaMiniController.getInstance().setXocDiaMiniInfoView(this);
        ...
        joinGame: function (info) {
            ...
            cc.XocDiaMiniController.getInstance().updatePositionPlayerUI(this.positionsUI);
        },
        ...
        updateConnectionStatus: function (info) {
            if (this.positionsUI) {
                ...
                if (status === cc.XocDiaMiniConnectionStatus.REGISTER_LEAVE_GAME && accID === cc.LoginController.getInstance().getUserId()) {
        ...
        totalUserWin: function (amount) {
            ...
            this.lbTotalUserWin.font = cc.XocDiaMiniController.getInstance().getWinFont();
            //play fx thang
                ...
                this.xxPlayers.forEach(function (xxPlayer) {
                    if (xxPlayer.nickName === message[0]) {
                        xxPlayer.showEmotion(cc.ChatRoomController.getInstance().getIndexEmotion(message)
                            // >>>>>>> remotes/origin/Update-DgTiger-XocDiaMini
                            , message);
                    }
                });
            } else {
        ...
        updateTimer: function (time) {
            if (this.lbTimer) {
                ...
                if (timeInt > 0) {
                    ...
                    if (timeInt <= 5 && this.currentState === cc.XocDiaMiniState.BETTING) {
                    ...
                    if (this.currentState === cc.XocDiaMiniState.SHOW_RESULT) {
            ...
            switch (state) {
                ...
                case cc.XocDiaMiniState.BETTING: //54
                    if (this.currentState !== state) {
                        ...
                        cc.XocDiaMiniController.getInstance().setSID(info.SessionID);
                        ...
                        //cc.XocDiaMiniController.getInstance().draw(info.History.reverse());
                    }
                    break;
                ...
                case cc.XocDiaMiniState.SHAKING:
                    if (this.currentState !== state) {
                        this.updateSessionId(info.SessionID);
                        cc.XocDiaMiniController.getInstance().clearAllChip();
                        this.progressTimer.node.parent.active = false;
                        // this.startTimer(time);
                        this.resetPlayersResultUI();
                        this.lbTimer.node.color = cc.Color.WHITE;
