...
(function () {
    cc.ChatView = cc.Class({
        ...
        properties: {
            ...
            liveVideo: cc.WebView,
            channelId: 'taixiu',
        },

        onLoad: function () {
            cc.ChatController.getInstance().setChatView(this);
            this.liveVideo.node.active = false;
        ...
        onEnable: function () {
            ...
            this.onGetLiveGameConfig();
        },

        onGetLiveGameConfig:function(){
            var getLiveVideoCommand = new cc.GetLiveVideoCommand;
            getLiveVideoCommand.execute(this);
        },

        onGetLiveVideoCommandResponse: function (response) {
            if(response){
                ...
                if(response.ResponseCode == 1){
                    if(response.list.Status == 1){
                        this.liveVideo.node.active = true;
                        this.liveVideo.url =  response.list.VideoUrl;
                    }else{
                        this.liveVideo.node.active = false;
                    }
                }else{
